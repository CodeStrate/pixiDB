...
def test_neighbors_returns_correct_names(populated_hash):
    ...
    result = populated_hash.neighbors("Alice", csr, buf)
...
def test_neighbors_unknown_node_raises(populated_hash):
    ...
    with pytest.raises(KeyError):
        populated_hash.neighbors("Unknown", csr, buf)
...
def test_neighbors_no_outgoing_edges(populated_hash):
    ...
    csr.build_csr([(0, 2, {}), (0, 3, {}), (1, 3, {}), (2, 3, {})], num_nodes=4)
    result = populated_hash.neighbors("ML Paper", csr, buf)
    assert result == []
...
def test_neighbors_merges_csr_and_buf(populated_hash,
    ...
    csr = CSR()
    csr.build_csr([(0, 2, {})], num_nodes=4)  # Alice→GraphDB Paper only
    buf = CSRBuffer(threshold=1000)
    buf.hash = populated_hash
    buf.add_edge_to_buffer(edge_alice_ml)  # Alice→ML Paper in buffer
    result = populated_hash.neighbors("Alice", csr, buf)
    assert set(result) == {"GraphDB Paper", "ML Paper"}
...
def test_neighbors_after_compact_returns_all(populated_buffer):
    ...
    result = populated_buffer.hash.neighbors("Alice", populated_buffer.csr, populated_buffer)
    assert set(result) == {"GraphDB Paper", "ML Paper"}
...
def test_neighbors_buf_only_no_csr_edges_for_node(populated_hash, edge_bob_ml):
    ...
    csr = CSR()
    csr.build_csr([(0, 2, {})], num_nodes=4)  # only Alice→GraphDB in CSR
    buf = CSRBuffer(threshold=1000)
    buf.hash = populated_hash
    buf.add_edge_to_buffer(edge_bob_ml)
    result = populated_hash.neighbors("Bob", csr, buf)
